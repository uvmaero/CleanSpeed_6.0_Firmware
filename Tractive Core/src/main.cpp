...
void IOReadTask(void* pvParameters) {
    for (;;) {
        ...
        if (xSemaphoreTake(xMutex, (TickType_t)10) == pdTRUE) {
            ...
            tractiveCoreData.inputs.frontBrake =
                map(tmpFrontBrake, 265, 855, PEDAL_MIN,
            ...
            tractiveCoreData.sensors.coolingTempIn =
                map(tmpCoolingIn, 0, 2500, 0,
                    100);  // find thermistor values via testing

            int tmpCoolingOut = analogReadMilliVolts(COOLING_OUT_TEMP_PIN);
            tractiveCoreData.sensors.coolingTempOut =
...
void TWAIReadTask(void* pvParameters) {
    for (;;) {
        ...
        if (xSemaphoreTake(xMutex, (TickType_t)10) == pdTRUE) {
            ...
            // if rx queue is full clear it (this is bad, TODO: implement twai
            // message filtering)
            uint32_t alerts;
        ...
        if (xSemaphoreTake(xMutex, (TickType_t)10) == pdTRUE) {
            ...
            switch (tractiveCoreData.tractive.prechargeState) {
                ...
                case PRECHARGE_OFF:
                    ...
                    if (tractiveCoreData.sensors.imdFault == false &&
                        tractiveCoreData.sensors.bmsFault == false) {
...
void GetCommandedTorque() {
    ...
    switch (tractiveCoreData.tractive.driveMode) {
        ...
        default:
            ...
            // we don't want to send a torque command if we were in an undefined
            // state
            commandedTorque = 0;
            break;
    }
...
uint16_t TractionControl(uint16_t commandedTorque) {
    ...
    if (corneringTractionControlEnable || spinTractionControlEnable) {
        ...
        tractiveCoreData.tractive.tractionControlModifier +=
            (TRAC_CON_MOD_STEP * TRAC_CON_STEP_INCREASE_MOD);
    }
...
void PrintScheduler() {
    ...
    // print
    // SERIAL_DEBUG.printf("read io:[%s](%d)<%d Hz> | write io:[%s](%d)<%d Hz> |
    // read twai:[%s](%d)<%d Hz> | write twai:[%s](%d)<%d Hz> |
    // precharge:[%s](%d)<%d Hz> \r",
    //   taskStatesStrings.at(0), debugger.ioReadTaskCount,
    //   taskRefreshRate.at(0), taskStatesStrings.at(1),
    //   debugger.ioWriteTaskCount, taskRefreshRate.at(1),
    //   taskStatesStrings.at(2), debugger.twaiReadTaskCount,
    //   taskRefreshRate.at(2), taskStatesStrings.at(3),
    //   debugger.twaiWriteTaskCount, taskRefreshRate.at(3),
    //   taskStatesStrings.at(4), debugger.prechargeTaskCount,
    //   taskRefreshRate.at(4));

    SERIAL_DEBUG.printf(
        "uptime: %d | read io:<%d Hz> (%d) | write io:<%d Hz> (%d) | read "
