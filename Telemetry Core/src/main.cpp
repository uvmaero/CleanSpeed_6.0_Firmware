...
void setup() {
    ...
    if (xMutex != NULL) {
        // start tasks
        if (setup.ioActive) {
            xTaskCreatePinnedToCore(IOReadTask, "IO-Read", TASK_STACK_SIZE, NULL, tskIDLE_PRIORITY,
                                    &xHandleIORead, 0);
        }

        if (setup.i2cActive) {
            xTaskCreatePinnedToCore(TractiveReadTask, "Tractive-Read", TASK_STACK_SIZE, NULL,
                                    tskIDLE_PRIORITY, &xHandleTractiveRead, 0);
        }

        if (setup.rpiComActive && setup.loraActive) {
            xTaskCreatePinnedToCore(DataWriteTask, "Data-Write", TASK_STACK_SIZE, NULL,
                                    tskIDLE_PRIORITY, &xHandleDataWrite, 1);
        }

        if (setup.gpsActive && setup.imuActive) {
            xTaskCreatePinnedToCore(DataReadTask, "Data-Read", TASK_STACK_SIZE, NULL,
                                    tskIDLE_PRIORITY, &xHandleDataRead, 1);
        }
...
void PrintSchedulerDebug() {
    ...
    taskRefreshRate.push_back(debugger.serialWriteTaskCount -
                              debugger.serialWriteTaskPreviousCount);
    taskRefreshRate.push_back(debugger.tractiveReadTaskCount -
                              debugger.tractiveReadTaskPreviousCount);
    ...
    // print it
    SERIAL_DEBUG.printf(
        "uptime: %d | read io:<%d Hz> (%d) | read data:<%d Hz> (%d) | write data:<%d Hz> (%d) | "
        "tractive update: <%d Hz> (%d) \r",
        uptime, taskRefreshRate.at(0), debugger.ioReadTaskCount, taskRefreshRate.at(1),
        debugger.serialReadTaskCount, taskRefreshRate.at(2), debugger.serialWriteTaskCount,
        taskRefreshRate.at(3), debugger.tractiveReadTaskCount);
